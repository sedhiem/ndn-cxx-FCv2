...
public: // nested types
  ...
  public:
    explicit Error(const std::string &what) : name::Component::Error(what) {}
  };
...
* @endcode
*/
  explicit Function(const Block &wire);
...
* @sa https://named-data.net/doc/ndn-tlv/name.html#ndn-uri-scheme
* @note To print URI representation into a stream, it is more efficient to use
*``os << name``.
*/
...
*/
  ...
  /** @brief Make a deep copy of the name, reallocating the underlying memory
 * buffer
*/
...
public: // access
     ...
     */
  bool empty() const { return m_wire.elements().empty(); }

  /** @brief Get number of components
*/
  size_t size() const { return m_wire.elements_size(); }
...
*                    Use @p npos to get the PartialFunction until the end of
*this Function.
* @return a new PartialFunction containing the extracted components
*
* If iStartComponent is positive and indexes out of bounds, returns an empty
*PartialFunction.
* If iStartComponent is negative and indexes out of bounds, returns components
*starting from the
* beginning of the Function. If nComponents is out of bounds, returns the
*components until the end
* of this Function.
*/
  PartialFunction getSubFunction(ssize_t iStartComponent,
                                 size_t nComponents = npos) const;

  /** @brief Extract a prefix of the name
* @param nComponents Number of components; if negative, size()+nComponents is
*used instead
* @return a new Function containing the prefix
*                   the prefix up to name.size() - N. For example
*getPrefix(-1)
*                   returns the name without the final component.
*@return A new partial name
*/
  PartialFunction getPrefix(ssize_t nComponents) const {
    if (nComponents < 0)
...
*/
  Function &append(const Component &component) {
    m_wire.push_back(component);
    return *this;
  }

  /** @brief Append a component, copying from a zero-terminated byte string
* @param value a zero-terminated string; it will be constructed as a component
*as is
*              and will not be interpreted as a URI
* @note This overload is necessary to ensure unambiguity of
*``append("string")``, because both
*       Component and PartialFunction are implicitly convertible from
*zero-terminated byte string.
*       This overload ensures the string is constructed as a Component.
*/
  Function &append(const char *value) { return append(Component(value)); }

  /** @brief Append a component, copying from [@p value, @p value + @p
*valueLength)
* @return a reference to this name, to allow chaining
*/
  Function &append(const uint8_t *value, size_t valueLength) {
...
*More efficient
*                  implementation is available when @p Iterator additionally
*satisfies
...
* @return a reference to this name, to allow chaining
*/
  template <class Iterator> Function &append(Iterator first, Iterator last) {
...
* @param value a Block; if its TLV-TYPE is not FunctionComponent, it is nested
*into a FunctionComponent
* @return a reference to this name, to allow chaining
*/
  Function &append(const Block &value) {
...
*/
  Function &appendNumber(uint64_t number) {
...
*
* The component is encoded as a 1-octet marker, followed by a
*nonNegativeInteger.
*
* @return a reference to this name, to allow chaining
* @sa NDN Naming Conventions
*https://named-data.net/doc/tech-memos/naming-conventions.pdf
*/
  Function &appendNumberWithMarker(uint8_t marker, uint64_t number) {
...
*https://named-data.net/doc/tech-memos/naming-conventions.pdf
*/
  Function &appendVersion(uint64_t version) {
...
*https://named-data.net/doc/tech-memos/naming-conventions.pdf
*/
  Function &appendSegment(uint64_t segmentNo) {
...
*https://named-data.net/doc/tech-memos/naming-conventions.pdf
*/
  Function &appendSegmentOffset(uint64_t offset) {
...
*https://named-data.net/doc/tech-memos/naming-conventions.pdf
*/
  Function &appendTimestamp(const time::system_clock::TimePoint &timePoint) {
...
*https://named-data.net/doc/tech-memos/naming-conventions.pdf
*/
  Function &appendSequenceNumber(uint64_t seqNo) {
...
* @return a reference to this name, to allow chaining
*/
  Function &appendImplicitSha256Digest(const ConstBufferPtr &digest) {
...
* @return a reference to this name, to allow chaining
*/
  Function &appendImplicitSha256Digest(const uint8_t *digest,
                                       size_t digestSize) {
...
*/
  ...
  /** @brief Append a component
* @note This makes push_back an alias of append, giving Function a similar API
*as STL vector.
*/
  template <class T> void push_back(const T &component) { append(component); }

  /** @brief Remove all components
* @post empty() == true
*/
  void clear() { m_wire = Block(tlv::Function); }
...
public: // algorithms
      ...
      *
      * In plain words, successor of a name is the same name, but with its last
      *component
...
*
* This name is a prefix of @p other if the N components of this name are same
*as the first N
...
*/
  bool isPrefixOf(const Function &other) const;
...
*
* If the first components of each name are not equal, this returns a negative
*value if
* the first comes before the second using the NDN canonical ordering for name
* components, or a positive value if it comes after.  If they are equal, this
*compares
* the second components of each name, etc. If both names are the same up to
*the size
* of the shorter name, this returns a negative value if the first name is
*shorter than
* the second or a positive value if it is longer.  For example, if you
*std::sort gives:
* /a/b/d /a/b/cc /c /c/a /bb .
* This is intuitive because all names with the prefix /a are next to each
*other.
* But it may be also be counter-intuitive because /c comes before /bb
*according
...
*/
  int compare(const Function &other) const {
...
*
* This is equivalent to this->getSubFunction(pos1,
*count1).compare(other.getSubFunction(pos2, count2));
*/
  int compare(size_t pos1, size_t count1, const Function &other,
              size_t pos2 = 0, size_t count2 = npos) const;
...
template <> struct hash<ndn::Function> {
  size_t operator()(const ndn::Function &name) const;
};
